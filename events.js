...
window.EVENTS = [
  ...
  },
  {
    ...
    href: "https://school.gameaibook.org",
  ...
  },
  {
    ...
    href: "https://www.linkedin.com/posts/girlsonlystudy_goss-girlsonlystudyspace-gossrealtalk-activity-7465856696075087872-Dp9A?utm_source=share&utm_medium=member_desktop&rcm=ACoAADu6WeoBujphukq9w15kEDHX-8xVDP1U_iY",
  ...
  },
  {
    ...
    href: "https://www.lazioinnova.it/innovazione-aperta/iniziativa/zagarolo-game-city-2025/",
  ...
  },
  {
    ...
    href: "https://www.creative-valley.fr/business-tech-acceleration-program-for-gaming-naples-2025",
    img: "assets/creative-valley-2025.jpg",
  },
  {
    role: "Partecipant",
    title: "NASA Space Apps Challenge Rome 2023",
    type: "Hackathon",
    place: "Rome, IT",
    date: "Oct 2023",
    ...
    href: "https://www.spaceappschallenge.org/2023/find-a-team/fes/",
...
window.EVENT_ROLE = {
  Organizer: {
    c: "var(--cyan)",
    soft: "rgba(52,216,238,0.16)",
    glow: "rgba(52,216,238,0.50)",
  },
  Speaker: {
    c: "var(--violet)",
    soft: "rgba(176,124,255,0.16)",
    glow: "rgba(176,124,255,0.50)",
  },
  Partecipant: {
    c: "var(--violet)",
    soft: "rgba(176,124,255,0.16)",
...
(function () {
  ...
  window.eventCardHTML = function (e, opts) {
    ...
    const media = e.img
      ? `<img src="${esc(
          e.img
        )}" alt="" class="absolute inset-0 w-full h-full" style="object-fit:${
          isLogo ? "contain" : "cover"
        };${isLogo ? "padding:22%;" : ""}">`
      : `<div class="absolute inset-0 ph-stripes flex items-center justify-center" style="background:${
          r.soft
        }">
           <span class="display font-semibold tracking-tight" style="font-size:24px;color:${
             r.c
           };opacity:.85">${esc(e.role)}</span>
         </div>`;
    const desc =
      clamp > 0
        ? `<p class="text-[14px] leading-relaxed text-[color:var(--muted)] font-light mt-3" style="display:-webkit-box;-webkit-line-clamp:${clamp};-webkit-box-orient:vertical;overflow:hidden">${esc(
            e.desc
          )}</p>`
        : "";
    const meta = [e.type, e.place]
      .map((s) => (s || "").trim())
      .filter((s) => s && s !== "—")
      .join(" · ");
    return `
    ...
    } class="ev-card pcard glass flex flex-col group" style="border-radius:18px">
      <div class="glow" style="box-shadow:0 0 0 1px ${
        r.glow
      }, 0 26px 60px -22px ${r.glow}; border-radius:18px"></div>
      <div class="thumb relative overflow-hidden" style="aspect-ratio:16/10; background:var(--bg-2); border-radius:18px 18px 0 0">
        ${media}
        <span class="absolute top-3 left-3 mono text-[11px] px-2.5 py-1 rounded-full" style="background:rgba(7,8,14,0.6);border:1px solid var(--border);color:${
          r.c
        }">${esc(e.role)}</span>
      </div>
      <div class="relative z-10 p-5 flex flex-col flex-1">
        <div class="mono text-[12px] text-[color:var(--dim)]">${esc(
          e.date
        )}</div>
        <h3 class="display font-semibold text-[20px] leading-[1.12] tracking-tight mt-2 group-hover:translate-x-0.5 transition-transform">${esc(
          e.title
        )}</h3>
        <div class="mono text-[11px] text-[color:var(--dim)] mt-2">${esc(
          meta
        )}</div>
        ${desc}
        <span class="mt-4 inline-flex items-center gap-2 mono text-[12px]" style="color:${
          r.c
        }">View ${
      ext ? "↗" : "→"
    } <span class="group-hover:translate-x-1 transition-transform"></span></span>
      </div>
    </a>`;
  };
})();
